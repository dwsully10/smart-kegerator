...
#include "/home/pi/libfacerec/include/facerec.hpp"	//<-- modify to your path!
...
void RaspiCvCam::Init()
{
    ...
    QtConcurrent::run(createImages);
}

void RaspiCvCam::createImages()
{
    // Init OpenCV Stuff
...
static void video_buffer_callback(MMAL_PORT_T *port, MMAL_BUFFER_HEADER_T *buffer)
{
    ...
    if (buffer->length)
    {
        ...
        if (RaspiCvCam::FrameUpdatedCallback != NULL)
...
void RaspiCvCam::StartCamera(FrameCallback callback)
{
    if (CameraOn)
        return;
...
void RaspiCvCam::StopCamera()
{
    ...
    qDebug("Camera stopping! 2");

    destroyCamera();

    FrameUpdatedCallback = NULL;

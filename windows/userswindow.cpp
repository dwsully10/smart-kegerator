...
#include <windows/trainingwindow.h>
...
void UsersWindow::updateUsersList()
{
    ...
    for(unsigned int i=0; i<User::UsersList.size(); i++)
    {
        ...
        QString str = QString("%1").arg(user->Name.c_str());
        users.push_back(str.toStdString());
    }

    ui->scrollPane->SetOptions(users);
}

void UsersWindow::on_trainButton_clicked()
{
    //QtConcurrent::run(FacialRecognitionManager::Train);
    TrainingWindow::ShowWindow(selectedUser);
    //hide();
}

void UsersWindow::on_poursButton_clicked()
{
    if (selectedUser == NULL)
        return;

    HistoryWindow::ShowWindow();
    HistoryWindow::Instance->SetUser(selectedUser);
    hide();
}
...
void UsersWindow::setSelectedIndex(int index)
{
    ...
    {
        selectedUser = NULL;
        ui->infoLabel->setText("Select a user");
        //ui->learningGroupBox->setVisible(false);
    }
    else
    {
        ...
        double paid = 0;
        vector<string> paymentStrings;

        for(int i=Payment::PaymentsByUserId[selectedUser->Id].size()-1; i >= 0; i--)
        {
            Payment* payment = Payment::PaymentsByUserId[selectedUser->Id][i];
            paid += payment->Amount;
            QString paymentText = QString("Paid: $%1\n%2").arg(QString::number(payment->Amount, 'f', 2)).arg(Utilities::GetFullTimeString(payment->Time).c_str());
            paymentStrings.push_back(paymentText.toStdString());
        }

        ui->paymentsScrollPane->SetOptions(paymentStrings);
        ...
        ui->nameLabel->setText(QString("%1").arg(selectedUser->Name.c_str()));
        ui->owedLabel->setText(QString("%1: $%2").arg(owed >= 0 ? "Owed" : "Credit").arg(QString::number(abs(owed), 'f', 2)));
        ...
        //ui->learningGroupBox->setVisible(true);
    }
...
{
    ui->prevImageButton->setEnabled(userPhotoIndex > 0);
    if (selectedUser != NULL && selectedUser->ImagePaths.size() > 0)
    {
        ui->imageCountLabel->setText(QString("%1/%2").arg(userPhotoIndex + 1).arg(selectedUser->ImagePaths.size()));
        ui->nextImageButton->setEnabled(userPhotoIndex < selectedUser->ImagePaths.size() - 1);
        string location = selectedUser->ImagePaths[userPhotoIndex];
        ui->userImageLabel->setPixmap(QPixmap(QString(location.c_str())).scaled(ui->userImageLabel->size(), Qt::KeepAspectRatio, Qt::SmoothTransformation));
    }
    else
    {
...
void UsersWindow::on_startCameraButton_clicked()
{
    ...
    RaspiCvCam::StartCamera(&handleFrame2);
...
{
    //ui->learningImageLabel1->setPixmap(QPixmap::fromImage(faceImage));
    //ui->learningImageLabel2->setPixmap(QPixmap::fromImage(croppedImage));
    //ui->learningStatusLabel->setText(statusText);
    updateLearningButtons();
}
...
{
    string location = QString(Settings::GetString("userPhotosLocation").c_str()).arg(selectedUser->Name.c_str()).arg(selectedUser->NextImageId++).toStdString();
...
{
    /*
    ui->startSearchButton->setText(QString(searching ? "Searching..." : "Start Search"));
    ui->saveImageButton->setEnabled(selectedUser != NULL && foundCroppedImage && searching == false);
    ui->startSearchButton->setEnabled(RaspiCvCam::CameraOn);

    ui->startCameraButton->setText(QString(RaspiCvCam::CameraOn ? "Stop Camera" : "Start Camera"));

    if (RaspiCvCam::CameraOn == false)
    {
        ui->learningStatusLabel->setText(QString("Stopped"));
        ui->learningImageLabel1->clear();
        ui->learningImageLabel2->clear();
    }
    */
}

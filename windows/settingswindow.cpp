    ...
    ui(new Ui::SettingsWindow)
{
    ...
    ledPin = Settings::GetInt("cameraLEDsPin");
...
{
    ui->option1Button->setText(QString("Turn LED %1").arg(ledState ? "Off" : "On"));
    ui->option2Button->setText(QString(App::Fullscreen ? "Go Windowed" : "Go Fullscreen"));
    ui->option4Button->setText(QString(Settings::GetBool("facialRecEnabled") ? "Recognition: On" : "Recognition: Off"));
}
...
{
    string userName = "";
    if (KeyboardDialog::TryGetString(userName, "Enter User Name") == false)
        return;
...
void SettingsWindow::on_setTimeButton_clicked()
{
    ...
    if (KeyboardDialog::TryGetString(dateTime, "Set Date/Time (24hr)", dateTime, NUMBER_DATETIME_INPUT, true) == false) return;
    ...
    MainWindow::Instance->ResetGraph();
}
